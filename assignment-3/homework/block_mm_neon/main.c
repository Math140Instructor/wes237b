      ...
      for (int pBlock = 0; pBlock < P; pBlock += BLOCK_SIZE) {
        ...
        for (int r = rBlock; r < rEnd; ++r) {
          for (int p = pBlock; p < pEnd; ++p) {
            ...
            // Copy aValue into all four NEON lanes.
            int32x4_t aVector = vdupq_n_s32(aValue);

            // Process four columns simultaneously.
            for (; col + 4 <= colEnd; col += 4) {
              int32x4_t bVector = vld1q_s32(&input1->data[p * M + col]);

              int32x4_t resultVector = vld1q_s32(&result->data[r * M + col]);

              // resultVector += aVector * bVector
              resultVector = vmlaq_s32(resultVector, aVector, bVector);

              vst1q_s32(&result->data[r * M + col], resultVector);
            }

            // Handle the remaining 1–3 columns.
            for (; col < colEnd; ++col) {
              result->data[r * M + col] += aValue * input1->data[p * M + col];
            }
          }
